...
export const editLecture = async (req, res) => {
    try {
        ...
        // Ensure the course still has the lec id if it was not already present
        const course = await Course.findById(courseId);
        ...
        return res.status(200).json({
            lecture,
            message:"Lecture upload successfuly."
        })
    } catch (error) {
        console.log(error);
        return res.status(500).json({
...
export const removeLecture = async(req, res) =>{
    try {
        ...
        // del lec from clodinary as well
        if(lecture.publicId){
            await deleteVideoFromCloudinary(lecture.publicId);
        }

        // remove lec reference from associated course
        await Course.updateOne(
            {lectures: lectureId}, // find the course that contains that lec
            {$pull: {lectures:lectureId}} // Remove the lecture id from that lectures array of course 
        )
